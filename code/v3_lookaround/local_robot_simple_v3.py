...
MEMORY_DISTANCE_CHECK_INTERVAL = 10.0   # 5 秒内检查距离是否缩短
...
class SharedState:
    def __init__(self):
        ...
        # 推理短期记忆：目标锁定后一段时间内若新推理为反方向则保留原方向
        self.last_locked_uv = None      # 最近一次 Target Locked 的 (u, v)
        self.last_locked_time = None   # 最近一次锁定的时间
...
def get_depth_at_uv(u, v, depth_img):
    """目标点 5x5 邻域深度：取最小值（最近点），便于走到目标附近再停；中位数易偏大导致提前停。"""
...
def get_depth_at_uv_with_position(u, v, depth_img):
    """与 get_depth_at_uv 相同逻辑，但返回 (深度值, 取深度的像素u, 取深度的像素v)，用于第一视角可视化 5x5 与最终取深度的位置。"""
    u_idx = int(np.clip(u, 0, IMG_WIDTH - 1))
    v_idx = int(np.clip(v, 0, IMG_HEIGHT - 1))
...
# ============== 6. FBE 前沿探索：获取 2-3 米外的随机可达点（同层约束）==============
def get_explore_waypoint(sim, curr_pos):
    """使用 pathfinder 获取距离当前位置 2-3m 的随机可达点作为探索点；仅接受与当前同层的点，避免跨楼层过远。"""
            ...
            try:
                ...
                with shared_state.lock:
                    ...
                    if j.get('status') == 'success':
                        ...
                        new_u, new_v = j['u'], j['v']
                        # 短期记忆：若最近曾锁定目标，且新推理方向与锁定方向相反，则忽略新结果、继续沿原方向
                        now = time.time()
...
def main():
    ...
    current_goal_3d_raw = None   # 模型所指的原始 3D（未 snap），用于俯视图显示，与真实目标一致
    goal_set_at = None
    stuck_count = 0
    step_count = 0
        ...
        while True:
            ...
            if uv:
                ...
                if depth is not None:
                    ...
                    u_lo = max(0, u_idx - 2)
                    u_hi = min(IMG_WIDTH, u_idx + 3)
                    v_lo = max(0, v_idx - 2)
                    v_hi = min(IMG_HEIGHT, v_idx + 3)
